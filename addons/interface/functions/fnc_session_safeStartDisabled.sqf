...
(EXTENSION callExtension ["backend:session:safe_start_disabled", [_session, _armaSession, _missionName, _worldName, _orbat]]) params ["_result", "_returnCode", "_errorCode"];
...
if (_returnCode != 0) exitWith {
    ERROR_2("Failed to alert safe start end (return %1), message: '%2'",_returnCode,_result);
    false
};
